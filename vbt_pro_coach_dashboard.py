...
# 前端显示刷新策略常量（仅影响 Streamlit 图像刷新，不影响后端分析）
HIGH_FREQ_MIN_INTERVAL = 0.06
LOW_FREQ_MIN_INTERVAL = 0.60
AUTO_DEGRADE_LAT_MS = 120.0
AUTO_RECOVER_LAT_MS = 80.0
AUTO_DEGRADE_FPS = 8.0
AUTO_RECOVER_FPS = 12.0
...
def _get_display_config_from_sidebar() -> str:
    ...
    st.sidebar.caption("自动：根据当前负载自动切换显示频率")
    st.sidebar.caption("高频显示：适合网络和设备状态良好时")
    st.sidebar.caption("低频显示：适合远程热点或弱网络环境")
    return selected_mode


def _get_video_display_width() -> int:
    """侧边栏视频显示宽度配置，仅影响前端展示尺寸。"""
    st.sidebar.subheader("🎥 视频显示尺寸")
    ...
    preset_width = VIDEO_SIZE_PRESETS.get(preset, 640)
    custom_width = st.sidebar.slider(
        "视频显示宽度 (px)",
        min_value=560,
        max_value=1280,
        value=int(st.session_state.get("video_display_width", preset_width)),
        step=40,
        key="video_display_width_slider",
    )
    final_width = int(custom_width or preset_width)
    st.sidebar.caption(f"当前显示宽度：{final_width}px（仅影响 Dashboard 展示，不影响采集/识别）")
    st.session_state["video_display_width"] = final_width
    return final_width


def _resize_frame_for_display(frame_rgb, target_width: int):
    if frame_rgb is None or target_width <= 0:
        return frame_rgb
    ...
    return frame_rgb if target_width == w else __import__("cv2").resize(frame_rgb, (target_width, target_height))


def _should_refresh(last_ts: float, interval: float, now_ts: float) -> bool:
    return (now_ts - float(last_ts or 0.0)) >= interval
...
def _get_current_user_name_safe() -> str:
    """实时结算阶段兜底取用户名，避免 NameError 中断流程。"""
    try:
        name = (
        ...
        pass
    try:
        from vbt_runtime_config import get_current_user_name as _runtime_get_current_user_name
        return _runtime_get_current_user_name()
    except Exception:
        return "default_user"


def get_current_user_name() -> str:
    """Dashboard 层兼容入口，避免 realtime finalize 中引用未定义函数。"""
    return _get_current_user_name_safe()


def _build_metric_snapshot(stats: dict, rep_velocities: list[float]) -> tuple:
    reps = int(stats.get("reps", 0) or 0)
    cur_mean = rep_velocities[-1] if rep_velocities else _safe_float(stats.get("current_vel"), 0.0)
    best_mean = max(rep_velocities) if rep_velocities else _safe_float(stats.get("best_vel"), 0.0)
    v_loss = ((best_mean - cur_mean) / best_mean * 100) if (rep_velocities and best_mean > 0) else _safe_float(stats.get("velocity_loss_pct"), 0.0)
    fps = round(_safe_float(stats.get("fps"), 0.0), 1)
...
def _render_pose_diag_block(container, pd_info: dict) -> None:
    with container.container():
...
def _build_pose_snapshot(pd_info: Optional[dict]) -> tuple:
    if not pd_info:
        return ()
    return (
        round(_safe_float(pd_info.get("score"), 100.0), 1),
        round(_safe_float(pd_info.get("knee_ratio"), 1.0), 3),
        bool(pd_info.get("knee_valgus")),
        round(_safe_float(pd_info.get("torso_angle"), 0.0), 2),
        bool(pd_info.get("torso_lean_warning")),
        round(_safe_float(pd_info.get("hip_x_drift_ratio"), 0.0), 4),
        bool(pd_info.get("stability_warning")),
    )


def _render_fatigue_block(container, fat_assess, fat_pred) -> None:
    with container.container():
        if fat_assess:
...
def _render_debug_block(container, debug_info: dict) -> None:
    with container.container():
        c1, c2, c3, c4, c5, c6 = st.columns(6)
        c1.metric("Video/s", str(int(debug_info.get("video_updates", 0))))
        c2.metric("Metric/s", str(int(debug_info.get("metric_updates", 0))))
        c3.metric("Chart/s", str(int(debug_info.get("chart_updates", 0))))
        c4.metric("History", str(int(debug_info.get("history_len", 0))))
        c5.metric("RepΔ", "Yes" if debug_info.get("rep_changed") else "No")
        c6.metric("Loop", f"{float(debug_info.get('loop_ms', 0.0)):.1f} ms")


def _build_fatigue_snapshot(fat_assess, fat_pred) -> tuple:
    assess_part = ()
    pred_part = ()
    if fat_assess:
        assess_part = (
            fat_assess.status,
            fat_assess.message,
            round(_safe_float(fat_assess.velocity_loss_pct), 1),
            round(_safe_float(fat_assess.threshold_pct), 1),
        )
    if fat_pred:
        pred_part = (
            round(_safe_float(fat_pred.predicted_next_v), 3),
            round(_safe_float(fat_pred.predicted_loss_pct), 1),
            round(_safe_float(fat_pred.confidence), 3),
            bool(fat_pred.failure_warning),
            str(fat_pred.message),
            len(getattr(fat_pred, "predicted_velocities", []) or []),
        )
    return assess_part + pred_part


def _init_display_controller_state(key: str, selected_mode: str, reset: bool = False) -> dict:
    """初始化/获取显示调度状态。"""
    state_key = f"_display_controller_{key}"
    if reset or state_key not in st.session_state:
        st.session_state[state_key] = {
            "selected_mode": selected_mode,
            "effective_mode": "高频显示",
            "last_render_ts": 0.0,
            "render_interval": HIGH_FREQ_MIN_INTERVAL,
            "ema_fps": None,
            "ema_latency": None,
            "degrade_count": 0,
            "recover_count": 0,
...
def _display_status_text(state: dict) -> str:
    """生成当前显示模式文本。"""
    selected = state.get("selected_mode", "自动")
    effective = state.get("effective_mode", "高频显示")
    if selected == "自动":
        if effective == "低频显示":
            return "低频显示（自动降级）"
        return "高频显示（自动）"
...
def _update_ema(prev: Optional[float], current: float) -> float:
    if prev is None:
        return current
    return (EMA_ALPHA * current) + ((1 - EMA_ALPHA) * prev)


def _update_display_controller(state: dict, stats: dict, selected_mode: str) -> bool:
    """基于 stats 更新显示调度状态，返回是否发生模式切换。"""
    state["selected_mode"] = selected_mode
    fps = float(stats.get("fps") or 0.0)
    latency = float(stats.get("latency_ms") or 0.0)

    state["ema_fps"] = _update_ema(state.get("ema_fps"), fps)
    state["ema_latency"] = _update_ema(state.get("ema_latency"), latency)

    prev_effective_mode = state.get("effective_mode", "高频显示")

    if selected_mode == "高频显示":
        state["effective_mode"] = "高频显示"
        state["degrade_count"] = 0
        state["recover_count"] = 0
    elif selected_mode == "低频显示":
        state["effective_mode"] = "低频显示"
        state["degrade_count"] = 0
        state["recover_count"] = 0
    else:
        # 自动模式：基于 EMA + 滞回计数做切换
        ema_fps = state.get("ema_fps") or 0.0
        ema_latency = state.get("ema_latency") or 0.0

        should_degrade = (ema_latency > AUTO_DEGRADE_LAT_MS) or (ema_fps < AUTO_DEGRADE_FPS)
        should_recover = (ema_latency < AUTO_RECOVER_LAT_MS) and (ema_fps > AUTO_RECOVER_FPS)

        if state.get("effective_mode", "高频显示") == "高频显示":
            if should_degrade:
                state["degrade_count"] = state.get("degrade_count", 0) + 1
            else:
                state["degrade_count"] = 0
            state["recover_count"] = 0

            if state["degrade_count"] >= AUTO_DEGRADE_COUNT:
                state["effective_mode"] = "低频显示"
                state["degrade_count"] = 0
                state["last_switch_notice"] = "检测到链路负载较高，已自动切换到低频显示"
            ...
            if state["recover_count"] >= AUTO_RECOVER_COUNT:
                state["effective_mode"] = "高频显示"
                state["recover_count"] = 0
                state["last_switch_notice"] = "链路状态恢复，已自动切回高频显示"
                state["last_switch_ts"] = time.time()

    state["render_interval"] = (
        HIGH_FREQ_MIN_INTERVAL
        if state.get("effective_mode") == "高频显示"
        else LOW_FREQ_MIN_INTERVAL
    )

    return prev_effective_mode != state.get("effective_mode")


def _should_render_frame(state: dict) -> bool:
    """是否应该向前端推送当前帧。"""
    now = time.time()
    last_render_ts = float(state.get("last_render_ts") or 0.0)
    interval = float(state.get("render_interval") or HIGH_FREQ_MIN_INTERVAL)
    if (now - last_render_ts) >= interval:
        state["last_render_ts"] = now
        return True
    return False


def _render_mode_badge(placeholder, state: dict) -> None:
    """渲染当前生效显示模式与关键状态。"""
    mode_text = _display_status_text(state)
    interval = float(state.get("render_interval") or HIGH_FREQ_MIN_INTERVAL)
    ema_fps = state.get("ema_fps")
    ema_latency = state.get("ema_latency")

    with placeholder.container():
        st.caption(f"当前显示模式：{mode_text}")
        if ema_fps is not None and ema_latency is not None:
            st.caption(
                f"刷新间隔: {interval:.2f}s | EMA FPS: {ema_fps:.1f} | EMA Latency: {ema_latency:.0f} ms"
            )
...
def _render_dual_mode_sidebar() -> tuple[str, int, float]:
    ...
    if is_bodyweight:
        ...
        for i, kg in enumerate(sorted(PLATE_SPECS.keys())):
            with plate_cols[i]:
                if st.button(f"+{kg}", key=f"plate_{kg}", width="stretch"):
                    ...
                    if len(plates) < MAX_PLATES_PER_SIDE:
                        ...
                        st.rerun()
        with plate_cols[5]:
            if st.button("清空", key="plate_clear", width="stretch"):
    ...
    if st.sidebar.button("保存配置", width="stretch", key="save_full_config"):
        save_runtime_config(load_kg=load_kg, user_name=user_name, user_height_cm=user_height)
        st.session_state["user_name"] = user_name
        st.sidebar.success("配置已保存")

    st.sidebar.markdown("---")
    selected_display_mode = _get_display_config_from_sidebar()
    st.session_state["selected_display_mode"] = selected_display_mode
    video_display_width = _get_video_display_width()
    st.session_state["video_display_width"] = video_display_width
...
def _render_mode_a_realtime() -> None:
    ...
    with col_ctrl:
        if not is_running:
            if st.button("▶ 开始采集", type="primary", width="stretch", key="rt_start"):
                st.session_state["rt_running"] = True
                st.session_state["realtime_running"] = True
                st.session_state["recording_active"] = bool(record_on)
                st.session_state["rt_user_stop_requested"] = False
    ...
    try:
        from vbt_cv_engine import process_squat_video_realtime_light
        import cv2
    ...
    try:
        ...
        gen = process_squat_video_realtime_light(
        ...
        for frame_bgr, stats in gen:
            ...
            if target_reps > 0 and rc >= target_reps and not st.session_state.get("rt_target_reached_notice_shown", False):
                status_slot.info("已达到目标次数，请点击 Stop 结束本组。")
                st.session_state["rt_target_reached_notice_shown"] = True
            will_finalize = bool(should_finalize_on_exit or st.session_state.get("rt_user_stop_requested", False))
            if (now_ts - last_dash_rt_log_ts) >= 1.0:
                logger.info(
                    "[DASH_RT] frame=%s rep=%s recording=%s target_reps=%s auto_stop=%s will_finalize=%s session_running=%s",
                    stats.get("frame_id", "na"),
                    rc,
                    bool(st.session_state.get("recording_active", False)),
                    target_reps,
                    auto_stop_on_target,
                    will_finalize,
                    bool(st.session_state.get("rt_running", False)),
                )
                last_dash_rt_log_ts = now_ts
            rep_changed = False
            if rc > last_rep_count and rc > 0:
                old_rc = last_rep_count
                rep_value = stats.get("inst_velocity")
                if rep_value is None:
                    rep_value = stats.get("current_vel", 0.0)
                rep_velocities.append(float(rep_value or 0.0))
                rep_velocities = rep_velocities[-30:]
                ...
                logger.info(
                    ...
                    target_reps,
                    auto_stop_on_target,
                    bool(should_finalize_on_exit or st.session_state.get("rt_user_stop_requested", False)),
                )
            ...
            if debug_elapsed >= 1.0:
                debug_info = {
                    "video_updates": video_update_count,
                    "metric_updates": metric_update_count,
                    "chart_updates": chart_update_count,
        ...
        if last_frame_rgb is not None:
            video_slot.image(_resize_frame_for_display(last_frame_rgb, video_display_width), channels="RGB")
        ...
        if gen is not None:
            try:
                ...
                pass
        user_stop_requested = bool(st.session_state.get("rt_user_stop_requested", False))
        recording_was_active = bool(st.session_state.get("recording_active", False))
        should_finalize_on_exit = bool(user_stop_requested)
        finalize_reason = "user_stop" if user_stop_requested else None
        st.session_state["rt_running"] = False
        st.session_state["realtime_running"] = False
        st.session_state["recording_active"] = False
    ...
    if rep_velocities:
        with summary_slot.container():
            ...
            if should_finalize_on_exit and not st.session_state.get(_fin_key, False):
                try:
                    logger.info(
                        "[DASH_FINALIZE_ENTER] reason=%s rep=%s recording=%s user_clicked_stop=%s auto_stop=%s",
                        finalize_reason or "unknown",
                        len(rep_velocities),
                        recording_was_active,
                        bool(st.session_state.get("rt_user_stop_requested", False)),
                        auto_stop_on_target,
                    )
                    from vbt_cv_engine import _finalize_set_from_velocities
                    from vbt_analytics_pro import DB_PATH as _db
                    from vbt_runtime_config import get_current_load_kg
                    safe_user_name = st.session_state.get("user_name") or "Guest"
                    ...
                    print(f"[Realtime finalize failed] {type(_fe).__name__}: {_fe}", flush=True)
                    traceback.print_exc()
                    st.session_state["last_finalize_error"] = str(_fe)
                    logger.warning("[DASH_FINALIZE_ERROR] reason=%s err=%s", finalize_reason or "unknown", _fe)
                    logger.warning("Realtime finalize failed: %s", _fe)
                    st.warning(f"实时结算异常（已忽略，不影响采集主流程）: {_fe}")
...
def _render_mode_b_upload() -> None:
    ...
    start_btn = st.button("开始分析", type="primary", width="stretch", key="start_analysis_btn")
