...
# --- ROM 完成度计算（供 vbt_cv_engine 每帧调用）---

NOISE_THRESHOLD_PCT = 0.10  # 忽略小于总高度 10% 的微小晃动


def compute_standing_baseline(hip_y_samples: list[float], num_frames: int = 5) -> Optional[float]:
    """
    从分析开始的前 5 帧内锁定「初始站立髋部高度」作为 0% 位移基准。
    Y 轴向下为正，站立时 hip_y 较小。
    """
    if not hip_y_samples or len(hip_y_samples) < max(3, num_frames // 2):
        return None
    arr = np.asarray(hip_y_samples[:num_frames], dtype=float)
    return float(np.median(arr))


def compute_realtime_rom_percent(
    ...
    """
    实时 ROM 完成度% = (current_hip_y - y_lowest) / (standing_height - y_lowest) * 100。
    Y 向下为正：站立时 hip_y 小，最低点 y_lowest 大。100% = 回到站立，0% = 在最低点。
    排除脏数据：若总位移范围 < 阈值，或当前位移占比 < 10%，返回 None 避免 0.1%-1% 乱跳。
    """
    denom = standing_height - y_lowest
    if abs(denom) < 1e-6:
        return None
    total_range = abs(denom)
    noise_threshold = total_range * noise_threshold_pct
    if total_range < noise_threshold:
        return None
    current_displacement = current_hip_y - y_lowest
    if abs(current_displacement) < noise_threshold:
        return None
    rom_pct = (current_displacement / denom) * 100.0
    return max(0.0, min(100.0, rom_pct))


def is_fatigue_70_realtime(v_base: float, v_current: float) -> bool:
    """
    实时疲劳判定：当 V_current < 0.7 × V_base 时判定为 70% 疲劳。
    用于主循环中实时 UI 渲染。
    Args:
        v_base: 基准速度 (m/s)，首个有效动作或历史最大速度
        v_current: 当前动作速度 (m/s)
    Returns:
        是否达到 70% 疲劳阈值
    """
    return v_base > 0 and v_current < 0.7 * v_base
